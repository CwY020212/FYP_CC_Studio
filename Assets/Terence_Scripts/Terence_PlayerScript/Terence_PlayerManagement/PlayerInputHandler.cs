...
public class PlayerInputHandler : MonoBehaviour, InputSystem_Actions.IPlayerActions
{
    ...
    private bool _tapToContinueInputThisFrame;
    private bool _decision1InputThisFrame;
    private bool _decision2InputThisFrame;
    private bool _decision3InputThisFrame;
    private bool _decision4InputThisFrame;

    private bool _tapToContinuePressedLastFrame;
    private bool _decision1PressedLastFrame;
    private bool _decision2PressedLastFrame;
    private bool _decision3PressedLastFrame;
    private bool _decision4PressedLastFrame;
    ...
    public void OnTapToContinue(InputAction.CallbackContext context)
    {
        if (context.performed)
        {
            _tapToContinueInputThisFrame = true;
        }
        else if (context.canceled)
        {
            _tapToContinueInputThisFrame = false;
        }
    }

    public void OnDecision1(InputAction.CallbackContext context)
    {
        if (context.performed)
        {
            _decision1InputThisFrame = true;
        }
        else if (context.canceled)
        {
            _decision1InputThisFrame = false;
        }
    }

    public void OnDecision2(InputAction.CallbackContext context)
    {
        if (context.performed)
        {
            _decision2InputThisFrame = true;
        }
        else if (context.canceled)
        {
            _decision2InputThisFrame = false;
        }
    }
    public void OnDecision3(InputAction.CallbackContext context)
    {
        if (context.performed)
        {
            _decision3InputThisFrame = true;
        }
        else if (context.canceled)
        {
            _decision3InputThisFrame = false;
        }
    }
    public void OnDecision4(InputAction.CallbackContext context)
    {
        if (context.performed)
        {
            _decision4InputThisFrame = true;
        }
        else if (context.canceled)
        {
            _decision4InputThisFrame = false;
        }
    }
    ...
    public bool GetTapToContinueInputDown()
    {
        bool result = _tapToContinueInputThisFrame && !_tapToContinuePressedLastFrame;
        _tapToContinuePressedLastFrame = _tapToContinueInputThisFrame;
        return result;
    }

    public bool GetDecision1InputDown()
    {
        bool result = _decision1InputThisFrame && !_decision1PressedLastFrame;
        _decision1PressedLastFrame = _decision1InputThisFrame;
        return result;
    }

    public bool GetDecision2InputDown()
    {
        bool result = _decision2InputThisFrame && !_decision2PressedLastFrame;
        _decision2PressedLastFrame = _decision2InputThisFrame;
        return result;
    }
    public bool GetDecision3InputDown()
    {
        bool result = _decision3InputThisFrame && !_decision3PressedLastFrame;
        _decision3PressedLastFrame = _decision3InputThisFrame;
        return result;
    }
    public bool GetDecision4InputDown()
    {
        bool result = _decision4InputThisFrame && !_decision4PressedLastFrame;
        _decision4PressedLastFrame = _decision4InputThisFrame;
        return result;
    }
}
